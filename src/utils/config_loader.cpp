...
Config load_config_file(std::string_view path, const Config& base) {
    ...
    if (auto* server = tbl["server"].as_table()) {
        ...
        if (auto v = (*server)["idle_timeout_seconds"].value<int64_t>())
            cfg.idle_timeout_seconds = static_cast<std::size_t>(*v);
        if (auto v = (*server)["max_clients"].value<int64_t>())
            cfg.max_clients = static_cast<std::size_t>(*v);
        if (auto v = (*server)["max_pending_write_bytes"].value<int64_t>())
            cfg.max_pending_write_bytes = static_cast<std::size_t>(*v);
    }

    if (auto* security = tbl["security"].as_table()) {
        if (auto v = (*security)["requirepass"].value<std::string>())
            cfg.requirepass = *v;
    }
